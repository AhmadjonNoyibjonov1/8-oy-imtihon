...
const loadState = () => {
  try {
    const serializedState = localStorage.getItem('spotifyState');
...
const initialState = {
  ...
  currentTrack: null,
  isPlaying: false,
  likedSongs: savedState?.likedSongs || [],
  likedPlaylists: savedState?.likedPlaylists || [],
  featuredPlaylists: [],
  audioPlayer: null,
  likedAlbums: [],
  shuffle: false,
...
const spotifySlice = createSlice({
  ...
  reducers: {
    ...
    toggleLikeSong: (state, action) => {
      ...
      localStorage.setItem('spotifyState', JSON.stringify({
    ...
    toggleLikePlaylist: (state, action) => {
      ...
      const index = state.likedPlaylists.findIndex(p => p.id === playlist.id);
      if (index !== -1) {
