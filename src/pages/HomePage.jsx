  ...
  if (loading) {
    return (
      <div className='flex flex-col gap-40 items-center justify-center h-screen'>
        <h2 className="text-[36px] text-red-800">Loading...</h2>
...
const MixCard = ({ title, playlist }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className="bg-white bg-opacity-10 flex items-center rounded-md overflow-hidden cursor-pointer hover:bg-opacity-20 transition-all duration-200"
    >
      <img
        src={playlist?.images[0]?.url || `/api/placeholder/80/80`}
        alt={title}
...
const PlaylistCard = ({ playlist }) => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { currentTrack, isPlaying, likedPlaylists, accessToken } = useSelector(
    (state) => state.spotify
  );
