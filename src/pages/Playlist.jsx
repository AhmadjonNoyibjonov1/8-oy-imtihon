...
import { fetchPlaylist, setCurrentTrack, setIsPlaying, toggleLikeSong, setPlaylist } from '../features/spotifySlice';
    ...
    if (accessToken && id) {
      if (currentPlaylist && currentPlaylist.id === id) {
        ...
        dispatch(fetchPlaylist({ accessToken, id }))
          .then((action) => {
            if (fetchPlaylist.fulfilled.match(action)) {
              dispatch(setPlaylist(action.payload.tracks.items.map(item => item.track)));
            }
            setLoading(false);
          });
      }
  ...
  if (loading) {
    return (
      <div className='flex flex-col gap-40 items-center justify-center h-screen'>
        <div className='text-[36px] text-red-900'>Yuklanmoqda...</div>
          ...
          <tbody>
            {tracks.map((item, index) => {
              ...
              return (
                <tr 
                  key={`${song.id}-${index}`} 
                  className={`hover:bg-white/10 group rounded-md transition-colors cursor-pointer ${isCurrentlyPlaying ? 'bg-white/20' : ''}`}
                  onClick={() => handlePlayPause(song)}
                >
                  <td className="py-5 w-12">
                    <button onClick={(e) => { e.stopPropagation(); handlePlayPause(song); }} className="invisible group-hover:visible">
                      {isCurrentlyPlaying ? 
                  ...
                  <td className="text-gray-400 text-right pr-8">
                    <button onClick={(e) => { e.stopPropagation(); handleLike(song); }} className="mr-4 invisible group-hover:visible">
